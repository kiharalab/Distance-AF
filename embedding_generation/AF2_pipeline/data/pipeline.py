...
class DataPipeline:
  ...
  def __init__(self,
               ...
               template_searcher: TemplateSearcher=None,
  ...
  def process2(self, input_fasta_path: str, msa_output_dir: str, a3m=None, hhsearch_result= None) -> FeatureDict:
    with open(input_fasta_path) as f:
      input_fasta_str = f.read()
    input_seqs, input_descs = parsers.parse_fasta(input_fasta_str)
    if len(input_seqs) != 1:
    ...
    with open(a3m, 'r') as f:
      result = {'a3m': f.read()}
    #for single sequence prediction
    # with open(a3m, 'r') as f:
    #   t = f.read()
    # t = '\n'.join(t.splitlines()[:2]) + '\n'
    # result = {'a3m': t}
    #print(hhsearch_hits)
    bfd_msa = parsers.parse_a3m(result['a3m'])

    with open(hhsearch_result) as f:
        hhr = f.read()
    pdb_template_hits = self.template_searcher.get_template_hits(
        output_string=hhr, input_sequence=input_sequence)
  ...
  def process3(self, input_fasta_path: str, msa_output_dir: str) -> FeatureDict:
    """Runs alignment tools on the input sequence and creates features."""
    with open(input_fasta_path) as f:
      input_fasta_str = f.read()
    input_seqs, input_descs = parsers.parse_fasta(input_fasta_str)
    ...
    with open(f'{msa_output_dir}/pdb_hits.hhr') as f:
